...
@pytest.mark.parametrize("client_class,transport_class,transport_name", [
    ...
    (PrivateCatalogAsyncClient, transports.PrivateCatalogGrpcAsyncIOTransport, "grpc_asyncio"),
...
@pytest.mark.parametrize("client_class", [
    PrivateCatalogClient, PrivateCatalogAsyncClient
])
@mock.patch.object(PrivateCatalogClient, "DEFAULT_ENDPOINT", modify_default_endpoint(PrivateCatalogClient))
@mock.patch.object(PrivateCatalogAsyncClient, "DEFAULT_ENDPOINT", modify_default_endpoint(PrivateCatalogAsyncClient))
...
@pytest.mark.parametrize("client_class,transport_class,transport_name,grpc_helpers", [
    (PrivateCatalogClient, transports.PrivateCatalogGrpcTransport, "grpc", grpc_helpers),
    (PrivateCatalogAsyncClient, transports.PrivateCatalogGrpcAsyncIOTransport, "grpc_asyncio", grpc_helpers_async),
...
def test_search_catalogs_pager(transport_name: str = "grpc"):
    ...
    with mock.patch.object(
        ...
        assert all(isinstance(i, private_catalog.Catalog)
                   for i in results)
...
async def test_search_catalogs_async_pages():
    ...
    with mock.patch.object(
        ...
        pages = []
        async for page_ in (await client.search_catalogs(request={})).pages: # pragma: no branch
            pages.append(page_)
        for page_, token in zip(pages, ['abc','def','ghi', '']):
            assert page_.raw_page.next_page_token == token
...
def test_search_products_pager(transport_name: str = "grpc"):
    ...
    with mock.patch.object(
        ...
        results = list(pager)
        assert len(results) == 6
        assert all(isinstance(i, private_catalog.Product)
                   for i in results)
...
async def test_search_products_async_pages():
    ...
    with mock.patch.object(
        ...
        pages = []
        async for page_ in (await client.search_products(request={})).pages: # pragma: no branch
            pages.append(page_)
        for page_, token in zip(pages, ['abc','def','ghi', '']):
            assert page_.raw_page.next_page_token == token
...
def test_search_versions_pager(transport_name: str = "grpc"):
    ...
    with mock.patch.object(
        ...
        results = list(pager)
        assert len(results) == 6
        assert all(isinstance(i, private_catalog.Version)
                   for i in results)
...
async def test_search_versions_async_pages():
    ...
    with mock.patch.object(
        ...
        pages = []
        async for page_ in (await client.search_versions(request={})).pages: # pragma: no branch
            pages.append(page_)
        for page_, token in zip(pages, ['abc','def','ghi', '']):
            assert page_.raw_page.next_page_token == token
...
def test_search_catalogs_rest_interceptors(null_interceptor):
    transport = transports.PrivateCatalogRestTransport(
        credentials=ga_credentials.AnonymousCredentials(),
        interceptor=None if null_interceptor else transports.PrivateCatalogRestInterceptor(),
        )
    client = PrivateCatalogClient(transport=transport)
    with mock.patch.object(type(client.transport._session), "request") as req, \
         mock.patch.object(path_template, "transcode")  as transcode, \
         mock.patch.object(transports.PrivateCatalogRestInterceptor, "post_search_catalogs") as post, \
         mock.patch.object(transports.PrivateCatalogRestInterceptor, "pre_search_catalogs") as pre:
...
def test_search_products_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
         mock.patch.object(path_template, "transcode")  as transcode, \
         mock.patch.object(transports.PrivateCatalogRestInterceptor, "post_search_products") as post, \
         mock.patch.object(transports.PrivateCatalogRestInterceptor, "pre_search_products") as pre:
        pre.assert_not_called()
        post.assert_not_called()
...
def test_search_versions_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
         mock.patch.object(path_template, "transcode")  as transcode, \
         mock.patch.object(transports.PrivateCatalogRestInterceptor, "post_search_versions") as post, \
         mock.patch.object(transports.PrivateCatalogRestInterceptor, "pre_search_versions") as pre:
        pre.assert_not_called()
        post.assert_not_called()
...
# removed from grpc/grpc_asyncio transport constructor.
@pytest.mark.parametrize("transport_class", [transports.PrivateCatalogGrpcTransport, transports.PrivateCatalogGrpcAsyncIOTransport])
def test_private_catalog_transport_channel_mtls_with_adc(
    transport_class
):
    mock_ssl_cred = mock.Mock()
